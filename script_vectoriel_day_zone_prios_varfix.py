...
def rollback(steps=1, apply_bl=True):
    ...
    if initial_length > 0 and apply_bl: 
        banned_flight = np.hstack([itinerary[-1],['rollback']])
        blacklist = np.vstack([blacklist,banned_flight])
...
choice=[]
visited = [dep_zone]
blacklist = np.array([[None,None, None, None, None, None, None]])
...
while (time.time()-start) < time_to_solve or first_time : #----------while you still have time
    ...
    while tries != 0: #------------- while not back at home       
        ...
        while  int(day) < size-1: #----------- loop for every day
            ...
            if choices.size > 0:
                ...
                rank = choices[:,3].astype('float') * ( np.vectorize(zones_prio.get)(choices[:,5])** (size/300) )

                choices = choices[rank.argsort()]                 # sort by score
                choice = choices[min(one_off, len(choices)-1)]    # take one of the cheapest once
                one_off = 0                                                           
    ...
    if (sum_prices == 0 or new_price < sum_prices) and len(itinerary) == size : 
        ...
        # check if you're taking an expensive flight because of the wrong day
        deals = []
        ...
        if float(max_var) / float(sum_prices) > 0.05:
            banned_flight = winner[max_var]+['max_var']
            blacklist = np.vstack([blacklist,banned_flight])
            if dev: print('max_var banned')
        random_rewind = (size-max_var) + random.randint(0,max_var)
    else:
        blacklist = blacklist[blacklist[:,6]=='rollback']  
        random_rewind =  random.randint(0,max_var)
    ...
    if dev: print('new price: ', new_price, 'random rewind: ', random_rewind, 'time: '+str(round(time.time()-start,0)),"length: ",str(len(itinerary)))
